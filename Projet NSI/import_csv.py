...
def get_enonce_exercices(chapitre: str, lesson: str, nom: str) -> list:
    ...
    val = ""
    for e in table:
        if e["chapitre"] == chapitre and e["lecon"] == lesson and e["nom_exo"] == nom:
            val = e["enonce"]
    return val 
...
def get_name_exercices(chapitre: str, lesson: str) -> list:
    ...
    val = []
    for e in table:
        if e["chapitre"] == chapitre and e["lecon"] == lesson:
...
def add_chapter(chapitre: str) -> bool:
    ...
    if not chapitre in get_all_chapters():
        ...
        table.writerow({"chapitres":chapitre, "lst_lecons":[]})
        f.close()
...
def add_lesson_bis(chapitre: str ,path: str ,nom_lecon: str) -> None or str :
    ...
    with open("chapitres.csv", 'r') as f:
        ...
        for e in test:
            if e["chapitres"] == chapitre and not nom_lecon in e["lst_lecons"]:
            ...
            for e in r:
                if e["chapitre"] == chapitre and e["lecon"] == lesson and e["nom_exo"] == nom:
                    ...
                    with open("exercices.csv","a") as f:
                        w = csv.DictWriter(f, ["chapitre", "lecon", "nom_exo", "enonce"])
                        w.writerow({"chapitre": chapitre, "lecon": lesson, "nom_exo": nom, "enonce": enonce})
                        return True
    return False
...
def find_line_exo(chapitre: str, lesson: str, nom: str)-> int or None:
    with open("exercices.csv", 'r') as f:
        ...
        for e in test:
            ...
            elif e["chapitre"] == chapitre and lesson == e["lecon"] and nom is None:
                res.append(line_to_delete)
            elif e["chapitre"] == chapitre and e["lecon"] == lesson and e["nom_exo"] == nom:
                res.append(line_to_delete)
        f.close()
        return res
...
def delete_lesson(chapitre: str,lesson_name: str) -> bool:
    ...
    if res == lst:
        ...
        return modif_line(str(res), str(lst), "lst_lecons", "chapitres.csv", ";")
