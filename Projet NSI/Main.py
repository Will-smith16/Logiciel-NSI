...
from dictionnaire_cours_exos import *
...
def main_menu() -> None:
    ...
    def access_to_btn_lessons(event) -> None:
...
def lesson(lesson_pdf:str ,chapter_name: str, lesson_name: str) -> None:
    ...
    def access_to_ex(event) -> None:
        ...
        if not "Veuillez choisir un" in select: # Si l'élément n'est pas celui de base alors l'utilsateur accede a l'exo
            exercice(dict_exos[lesson_name][select],chapter_name,lesson_pdf,lesson_name)

    lst_exos : list= ["Veuillez choisir un exercice :"]
    for exo in dict_exos[lesson_name].keys():
        lst_exos.append(exo)
    if is_admin:
...
def exercice(pdf_exo: str, chapter_name: str, lesson_pdf:str, lesson_name:str) -> None:
